...
namespace Hakatonv2
{
    public partial class Form1 : Form
    {
        ...
        void CreateSessionLog()
        {
            ...
            session.Id = currentPatient.Sessions.Count + 1;
            session.DoctorNotes = "";
            session.GenerateSessionLog(suds.ToArray());
            session.AddSuds(suds.ToArray());
        ...
        private void ProcessIncomingMessage(string[] message)
        {
            ...
            Invoke(new Action(() =>
            {
                switch (command)
                {
                    ...
                    case "time_ended":
                        stream_button.Enabled = false;
                        pause_button.Enabled = false;
                        start_bp_button.Enabled = false;
                        start_emdr_button.Enabled = false;
                        stop_button.Enabled = true;
                        effewcts_button.Enabled = false;
                        CreateSessionLog();
                        DataBaseManager.SavePatient(currentPatient);
                        break;
        ...
        void ShowMainPanel()
        {
            ...
            using (FormSetTimeSession form = new FormSetTimeSession())
            {
    ...
    public class Sud
    {
        ...
        {
            return $"{sudId};{volume};{sudLvl};{sceneName}";
        }
    }
}
