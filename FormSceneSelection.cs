    ...
    public partial class FormSceneSelection : Form
    {
                ...
                switch (command)
                {
                    case "scene_uploaded":
                        if (isFileSending)
                        {
                            ...
                            {
                                if (SceneManager.scenes[i].Name == parts[1] || SceneManager.scenes[i].FileName == parts[1])
                                {
                                    SceneManager.scenes[i].IsAvailableOnQuest = true;
                                    SceneManager.scenesOnQuest.Add(SceneManager.scenes[i]);
                                    Debug.WriteLine("инит сцены");
                                }
                                break;
                            }

                            Scene nsC = new Scene();
                            nsC.IsAvailableOnQuest = true;
                            nsC.Name = parts[1];

                            SceneManager.scenes.Add(nsC);
                            SceneManager.scenesOnQuest.Add(nsC);
        ...
        private async void upload_scene_button_Click(object sender, EventArgs e)
        {
            ...
            if (openDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                NetworkManager.SendMessage("send_new_scene");
                isFileSending = true;
                fileSender.StartFileSender(filePath);
            }
        }
    }
}
