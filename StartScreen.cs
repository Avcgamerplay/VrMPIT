...
namespace Hakatonv2
{
    public partial class StartScreen : Form
    {
        ...
        public StartScreen()
        {
            ...
            uploadbase_button.BackgroundImageLayout = ImageLayout.Tile;
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                {
                    List <Patient> patientsLoaded = CSVManager.LoadFromCsv(openFileDialog.FileName);

                    for(int i = 0; i < patientsLoaded.Count; i++)
                    {
                        if(patientRepo.GetPatientById(patientsLoaded[i].Id) == null)
                        {
                            patientRepo.AddPatient(patientsLoaded[i]);
                            DataBaseManager.SaveNewPatient(patientsLoaded[i]);
                        }
                        else
                        {
                            Debug.WriteLine("такой есть");
                        }
                    }

                    Debug.WriteLine(patientsLoaded.Count);
